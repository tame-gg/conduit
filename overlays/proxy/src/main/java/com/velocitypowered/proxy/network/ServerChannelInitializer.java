  ...
  protected void initChannel(final Channel ch) {
    ...
    if (ch.remoteAddress() instanceof InetSocketAddress remote) {
      ...
      if (remoteAddress != null) {
        ...
        if (conduit.getBotFilter().isBlocked(remoteAddress)) {
          ch.close();
          return;
        }
        if (conduit.getThrottler().isThrottled(remoteAddress)) {
          conduit.getDiagnostics().recordThrottledConnection(remoteAddress.getHostAddress());
          ch.close();
          return;
        }
